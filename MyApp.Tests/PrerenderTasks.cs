...
namespace MyApp.Tests
{
    ...
    public class PrerenderTasks
    {
        ...
        void Render<TComponent>(params ComponentParameter[] parameters)
             where TComponent : IComponent
        {
            var component = Context.RenderComponent<TComponent>(parameters);
            var route = typeof(TComponent).GetCustomAttribute<Microsoft.AspNetCore.Components.RouteAttribute>()?.Template;
            if (string.IsNullOrEmpty(route))
                throw new Exception($"Couldn't infer @page for component {typeof(TComponent).Name}");

            var fileName = route.EndsWith("/") ? route + "index.html" : $"{route}.html";

            var writeTo = PrerenderDir.CombineWith(fileName);
